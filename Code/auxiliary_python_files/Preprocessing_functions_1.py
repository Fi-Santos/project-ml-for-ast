...
def get_dataset(file):
    """
    Reads an Excel file and organises the data into a hierarchical structure.

    Each sheet is split into:
    - raw data (brutos)
    - processed data (gvr)

    The final structure follows:
    dataset[data][concentration]["brutos/gvr"]
    ...
    -------
    dict
        Dictionary structured by date and concentration.
    """
    ...
    for nome in xl.sheet_names:
        ...
        indices = [i for i, col in enumerate(folha.columns) if "gray value" in str(col).lower()]
        ...
        if data not in ecoli_ampli:
            ecoli_ampli[data] = {}
    
        ecoli_ampli[data][conc] = {
            "brutos": brutos.copy(), 
            "gvr":    gvr.copy()   
        }
    return ecoli_ampli
...
def clean_Nas(dataset):
    ...
    for data, concs in dataset.items():
        for conc, datasets in concs.items():
            ...
            dataset[data][conc]["brutos"] = brutos[brutos.iloc[:, 1:].notna().any(axis=1)].reset_index(drop=True)
            dataset[data][conc]["gvr"]    = gvr[gvr.iloc[:, 1:].notna().any(axis=1)].reset_index(drop=True)
